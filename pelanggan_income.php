...
$halaman = "pelanggan_income"; // halaman
  ...
  <!-- KONTEN BODY AWAL -->
      ...
      <div class="box">
        <div class="box-header with-border">
          <h3 class="box-title">Pendapatan Oleh Pelanggan</h3>
...
$sqlb="SELECT SUM(total) AS total FROM $forward WHERE pelanggan='$pelanggan' AND tglsale BETWEEN '" . $dari . "' AND  '" . $sampai . "'";
$hasila=mysqli_query($conn,$sqlb);
$rowa=mysqli_fetch_assoc($hasila);
$total=$rowa['total'];


$sqlb="SELECT SUM(total-sudahbayar) AS totale FROM $forward WHERE pelanggan='$pelanggan' AND status LIKE '%belum%' AND tglsale BETWEEN '" . $dari . "' AND  '" . $sampai . "'";
$hasila=mysqli_query($conn,$sqlb);
$rowa=mysqli_fetch_assoc($hasila);
$belum=$rowa['totale'];
...
<?php 
    ...
    <div class="box">
        <div class="box-header with-border">
          <h3 class="box-title">Daftar Transaksi Pelanggan : <?php echo $nama;?> </h3>
...
<?php 
 ...
 $query1="SELECT *, sale.status as status from sale inner join pelanggan on sale.pelanggan=pelanggan.kode where pelanggan.nama LIKE '%$nama%' AND sale.tglsale BETWEEN '" . $dari . "' AND  '" . $sampai . "' ";
...
<tr>
            ...
            <td><?php  echo mysqli_real_escape_string($conn, number_format($fill['total'], $decimal, $a_decimal, $thousand).',-'); ?></td>
               <td><?php  echo mysqli_real_escape_string($conn, number_format($fill['sudahbayar'], $decimal, $a_decimal, $thousand).',-'); ?></td>
                  <td><?php  echo mysqli_real_escape_string($conn, number_format( ($fill['total']-$fill['sudahbayar']), $decimal, $a_decimal, $thousand).',-'); ?></td>
