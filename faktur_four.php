...
$judul="Invoice";
  ...
  $sql1="SELECT * FROM $tabel where nota='$nota'";
    ...
    $bnama=$row['biaya_nama'];
        $biaya=$row['biaya'];
               $diskon=$row['diskon_persen'];
        $pot=$row['diskon'];
           $perppn=$row['pajak_persen'];
              $ppn=$row['pajak'];
...
$numofpage=ceil($lop['countitem']/9);

if($lop['countitem']==9){
    $rowoflastpage=9;
} else {
    $rowoflastpage=$lop['countitem'] % 9;
}


$filler=9-$rowoflastpage;
...
<?php    } ?>
        ...
        <div style="width: 100%;padding-right: 10px;" class="col-md-12">
            ...
            <div class="row">
                ...
                <div class="col-lg-8" style="width: 30%;">
                    <h5 style="font-size: 20px;margin-bottom: 15px;">FAKTUR PENJUALAN</h5>

                    <h5 style="font-size: 12px;margin: 0;padding: 0;"><b><?php echo date('d-m-Y',strtotime($tgl));?></b></h5>

                    <p style="font-size: 12px;margin: 0;padding-top: 3px;;">Kasir: <?php echo $kasir;?></p>
            ...
            <table width="100%" border="0">
                ...
                <tr>
                    <th style="width: 80%;font-size: 12px;background: rgba(217,225,242,1.0)">Faktur:#<?php echo $nota;?></th>
                    <th></th>
                </tr>
            ...
            <table width="100%" border="1px">
                ...
                <tr>
                    ...
                    <td style="text-align:right"><?php echo number_format($rowa['harga']*$rowa['jumlah']);?></td>
                </tr>
    ...
    <?php if($i==$lastpage){?>
                ...
                <tr style="background: rgba(217,225,242,1.0);">
                    ...
                    <td colspan="2"><?php if($pot>0){?> Diskon (<?php echo $diskon;?>%) : <?php echo number_format($pot, $decimal, $a_decimal, $thousand).',-';?> <?Php } ?></td>
                    <td><?php if($perppn>0){?> PPN (<?php echo $perppn;?>%) : <?php echo number_format($ppn, $decimal, $a_decimal, $thousand).',-';?><?php } ?></td>
                    <td colspan="2">Jumlah</td>
                    <td colspan="1" style="text-align:right"><?php echo number_format(($pot+$total), $decimal, $a_decimal, $thousand).',-';?></td>
                </tr>
                <tr style="background: rgba(217,225,242,1.0);">
                     <td>&nbsp;</td>
                    <td colspan="3"><?php if($biaya>0){?> <?php echo $bnama;?> : <?php echo number_format($biaya, $decimal, $a_decimal, $thousand).',-';?><?php } ?></td>
...
<?php } else { ?>
 <td width="40%" align="center" valign="top">
<h5 style="font-size: 30px">LUNAS</h5>
</td>
