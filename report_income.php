...
?>
  ...
  <?php
$sqlbRow = SecurityBootstrap::queryOne(
    $conn,
    'SELECT (SUM(total) - SUM(keluar)) AS total FROM bayar WHERE nota IN (SELECT nota FROM transaksimasuk) AND tglbayar LIKE ?',
    's',
    [$tahun . '-' . $bulan . '-%']
);
$totalincome = $sqlbRow['total'] ?? 0;
