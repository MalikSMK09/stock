    ...
    if(isset($_POST["masuk"])){
       if($_SERVER["REQUEST_METHOD"] == "POST"){
         ...
         //            $upd=mysqli_query($conn,$sql3);
                   ...
                   mysqli_query($conn,"SET session sql_mode = ''");	 $sql2 = "insert into stok_masuk_daftar values( '$nota','$kode','$nama','$jumlah','')";
                     $insertan = mysqli_query($conn, $sql2);

                      if($insertan){

                    mysqli_query($conn,"SET session sql_mode = ''");	 $sql9="INSERT INTO mutasi VALUES('$usr','$today','$jam','$kode','$newstok','$jumlah','stok masuk','$nota','','pending')";
       ...
       if($_SERVER["REQUEST_METHOD"] == "POST"){
               ...
               $sup= mysqli_real_escape_string($conn, $_POST["supplier"]);
                $tgl= date('Y-m-d');
                  ...
                  mysqli_query($conn,"SET session sql_mode = ''");	  $sql2 = "insert into stok_masuk values( '$nota','$cab','$tgl','$sup','$usr','')";
                     $insertan = mysqli_query($conn, $sql2);
